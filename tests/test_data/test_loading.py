...
class TestLoading(object):
    ...
    @classmethod
    def setup_class(cls):
        cls.data_prefix = osp.join(osp.dirname(__file__), '../datas')
    ...
    def test_load_img(self):
        ...
        # no img_prefix
        results = dict(
            img_prefix=None, img_info=dict(filename='tests/datas/color.jpg'))
        transform = LoadImageFromFile()
        results = transform(copy.deepcopy(results))
        assert results['filename'] == 'tests/datas/color.jpg'
        assert results['ori_filename'] == 'tests/datas/color.jpg'
        assert results['img'].shape == (288, 512, 3)
