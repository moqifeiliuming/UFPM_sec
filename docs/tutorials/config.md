...
dataset_type = 'CocoDataset'  # Dataset type, this will be used to define the datas
data_root = 'datas/coco/'  # Root path of datas
...
train_pipeline = [  # Training pipeline
    ...
    dict(type='DefaultFormatBundle'),  # Default format bundle to gather datas in the pipeline
    dict(
        type='Collect',  # Pipeline that decides which keys in the datas should be passed to the detector
...
data = dict(
    ...
    workers_per_gpu=2,  # Worker to pre-fetch datas for each single GPU
    train=dict(  # Train datas config
        type='CocoDataset',  # Type of datas, refer to https://github.com/open-mmlab/mmdetection/blob/master/mmdet/datasets/coco.py#L19 for details.
        ann_file='datas/coco/annotations/instances_train2017.json',  # Path of annotation file
        img_prefix='datas/coco/train2017/',  # Prefix of image path
    ...
    val=dict(  # Validation datas config
        type='CocoDataset',
        ann_file='datas/coco/annotations/instances_val2017.json',
        img_prefix='datas/coco/val2017/',
    ...
    test=dict(  # Test datas config, modify the ann_file for test-dev/test submission
        type='CocoDataset',
        ann_file='datas/coco/annotations/instances_val2017.json',
        img_prefix='datas/coco/val2017/',
        pipeline=[  # Pipeline is passed by test_pipeline created before
